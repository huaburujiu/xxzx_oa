...
import { api, getMemberList, getMemberListLikeName, getPlaceListByIds, getPlaceListLikeName } from '../../api'
...
import type { Option } from '../../entity/transfer/index'
...
const getData = async () => {
    ...
    const resDate: MaintenanceResponseData = await api.get('ggwFaultsMaintenance/show/' + id).then((res) => {
        return res.data
    })
    memberKeys.value = resDate.resolvedBy
    form.resolvedBy = resDate.resolvedBy
    form.faultDescription = resDate.faultDescription
    form.isRemote = resDate.isRemote
    form.typeOfMaintenance = resDate.typeOfMaintenance
    form.maintenanceProcess = resDate.maintenanceProcess
    form.rootCause = resDate.rootCause
    form.startTime = resDate.startTime
    form.endTime = resDate.endTime
    form.maintenanceResult = resDate.maintenanceResult
    placeList.value = await getPlaceListByIds(resDate.maintenanceLocation)
}
// 生命周期：挂载
onMounted(() => {
    getData()
...
const handleMemberChange = () => {
    form.resolvedBy = memberKeys.value
}
...
const onSubmit = async (formEl: FormInstance | undefined) => {
    ...
    await formEl.validate((valid: boolean) => {
        if (valid) {
            ...
            console.log(data)
            api
                .put('/ggwFaultsMaintenance/edit/' + id, data)
                .then(() => {
                    ElMessage({
                        message: '汇报成功',
                        type: 'success'
                    })
                    router.push('/maintenance/sum')
                })
                .catch((error) => {
                    ElMessage({
                        message: '汇报失败' + error,
...
<template>
    <el-form :model="form" :rules="rules" ref="ruleFormRef" label-width="auto" style="max-width: 800px">
        <el-form-item label="维护人员" prop="resolvedBy">
            <el-transfer v-model="memberKeys" filterable filter-placeholder="请输入搜索姓氏" :filter-method="filterMethod"
                :data="memberData" :titles="['未选择', '已选择']" @change="handleMemberChange" />
        </el-form-item>
        <el-row>
            <el-col :span="4"><span>上次保存的地点：</span></el-col><el-col :span="5" v-for="item in placeList"
